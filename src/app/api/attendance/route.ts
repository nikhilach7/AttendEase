...
export async function GET(req: Request) {
  try {
    ...
    if (date) {
      const localDate = new Date(date); // Interpret the incoming date string as a local date
      ...
      console.log('Searching attendance from (local time):', localDate, 'UTC range:', start.toISOString(), 'to', end.toISOString());
    }
